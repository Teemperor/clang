...
bool isHashed(const std::string &DeclName, const std::string &code) {
  ...
  return Hash.findHash(StmtSequence(Finder.getDecl()->getBody(), &ASTContext)).Success;
}
...
bool isHashed(Stmt::StmtClass NeededStmtClass, const std::string &code) {
  ...
  return Hash.findHash(StmtSequence(Finder.getStmt(), &ASTContext)).Success;
}
...
bool compareStructure(const std::string &DeclNameA,
  ...
  auto HashSearchA = HashA.findHash(StmtSequence(
                                      FinderA.getDecl()->getBody(),
                                      &ASTContextA
                                    ));
  auto HashSearchB = HashB.findHash(StmtSequence(
                                      FinderB.getDecl()->getBody(),
                                      &ASTContextB
                                    ));
...
TEST(StmtFeature, Test) {
  ...
  StmtFeature FeatureA(StmtSequence(Finder.getDecl()->getBody(),
                                    &ASTUnitA->getASTContext()));

  Finder.TraverseTranslationUnitDecl(
        ASTUnitB->getASTContext().getTranslationUnitDecl());

  StmtFeature FeatureB(StmtSequence(Finder.getDecl()->getBody(),
                                    &ASTUnitB->getASTContext()));
