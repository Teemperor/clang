...
void CloneDetector::analyzeCodeBody(const Decl *D) {
  ...
  Sequences.push_back(StmtSequence(D->getBody(), D->getASTContext()));
}
...
size_t createHash(llvm::MD5 &Hash) {
  size_t HashCode;
  ...
  // Copy as much as possible of the generated hash code to the signature's hash
  // code.
...
size_t HashConstraint::saveHash(
  ...
  for (const Stmt *S : S->children()) {
    ...
    size_t ChildHash = saveHash(S, Context, StmtsByHash);
    assert(ChildHash);
    Hash.update(
    ...
    for (unsigned Length = 2; Length <= CS->size(); ++Length) {
      for (unsigned Pos = 0; Pos <= CS->size() - Length; ++Pos) {
        ...
        StmtsByHash.push_back(std::make_pair(
            createHash(Hash), StmtSequence(CS, Context, Pos, Pos + Length)));
      }
    }
  }

  size_t HashCode = createHash(Hash);
  StmtsByHash.push_back(std::make_pair(HashCode, StmtSequence(S, Context)));
  return HashCode;
}
...
void HashConstraint::constrain(
  ...
  for (CloneDetector::CloneGroup &Group : Sequences) {
    ...
    for (const StmtSequence &S : Group) {
      saveHash(S.front(), S.getASTContext(), StmtsByHash);
    }
    ...
    for (unsigned i = 0; i < StmtsByHash.size() - 1; ++i) {
      ...
      for (; i < StmtsByHash.size(); ++i) {
        ...
        if (PrototypeHash != StmtsByHash[i].first ||
          ...
          // the hash of the Current CloneSignature with itself in the 'if'
          // above.
          assert(i != 0);
          --i;
          break;
...
size_t MinComplexityConstraint::calculateStmtComplexity(
  ...
  if (Seq.holdsSequence()) {
    for (const Stmt *S : Seq) {
      Complexity +=
          calculateStmtComplexity(StmtSequence(S, Context), StartMacroStack);
    }
  } else {
...
void MatchingVariablePatternConstraint::constrain(
  ...
  filterClones(
    CloneGroups, [](const StmtSequence &A, const StmtSequence &B) {
      ...
      return PatternA.getPatternDifferences(PatternB) == 0;
    });
}

void CloneConstraint::filterClones(std::vector<CloneDetector::CloneGroup> &CloneGroups, std::function<bool (const StmtSequence &, const StmtSequence &)> Compare) {
  std::vector<CloneDetector::CloneGroup> Result;
